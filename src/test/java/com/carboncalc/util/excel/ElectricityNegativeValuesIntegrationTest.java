...
public class ElectricityNegativeValuesIntegrationTest {

    @Test
    public void preservesNegativeConsumptionThroughPerCenterAndTotal() throws Exception {
        ...
        Path prov = Files.createTempFile("prov-neg", ".xlsx");
        try (FileOutputStream fos = new FileOutputStream(prov.toFile())) {
            src.write(fos);
        }
        src.close();

        Path out = Files.createTempFile("elec-out-neg", ".xlsx");
        ...
        try (FileInputStream fis = new FileInputStream(out.toFile()); Workbook wb = new XSSFWorkbook(fis)) {
            ...
            String moduleLabel = spanish.containsKey("module.electricity") ? spanish.getString("module.electricity")
                    : "Electricidad";
            ...
            try {
                ...
                // Fallback: read detailed sheet numeric values and sum them
                ...
                for (int rr = det.getFirstRowNum() + 1; rr <= det.getLastRowNum(); rr++) {
                    ...
                    if (rrw == null)
                        continue;
                    Cell c = rrw.getCell(7); // consumo is at index 7 in provider layout
                    if (c != null && c.getCellType() == CellType.NUMERIC) {
                        sum += c.getNumericCellValue();
                    }
                }
                assertEquals(-1200.0, sum, 0.0001, "Fallback manual sum should be negative provider value");
            }

...
public class GasController {
    ...
    public void setView(GasPanel view) {
        this.view = view;
        loadStoredCups();
        // Populate gas types for the current year
        try {
            populateGasTypesForYear(this.currentYear);
    ...
    private void loadStoredCups() {
        // Load persisted CUPS and add only gas-type entries to the view
        try {
        ...
        // Ensure gas type selector shows values for the current year
        try {
            populateGasTypesForYear(this.currentYear);
        } catch (Exception ignored) {
        }
    ...
    private void generateExcelReport() {
        try {
            ...
            GasMapping mapping = view.getSelectedColumns();
            ...
            if (erpPath != null && erpSheet != null && view.getErpInvoiceNumberSelector().getSelectedItem() != null) {
                try (java.io.FileInputStream fis = new java.io.FileInputStream(erpPath)) {
                    org.apache.poi.ss.usermodel.Workbook erpWb = erpPath.toLowerCase().endsWith(".xlsx")
                    ...
                    if (sheet != null) {
                        ...
                        for (int r = sheet.getFirstRowNum(); r <= sheet.getLastRowNum(); r++) {
                            ...
                            for (Cell c : row) {
                                if (!getCellValueAsString(c).isEmpty()) {
                                    nonEmpty = true;
                                    break;
                                }
                            }
            ...
            GasExcelExporter.exportGasData(
